...
def load_model(model_type, data_type):
    ...
    elif data_type == 'usdjpy':
        if model_type == 'LSTM':
            best_params = best_params_lstm_usd_jpy
        elif model_type == 'CNN':
            best_params = best_params_cnn_usd_jpy
        elif model_type == 'CNN-LSTM':
            best_params = best_params_cnn_lstm_usd_jpy
        elif model_type == 'TimesNet':
            best_params = best_params_timesnet_usd_jpy
        elif model_type == 'Transformer':
            best_params = best_params_transformer_usd_jpy
        else:
            raise ValueError("Invalid model_type. Choose either 'LSTM' or 'CNN'.")
        
    elif data_type == 'wti':
        if model_type == 'LSTM':
            best_params = best_params_lstm_wti
        elif model_type == 'CNN':
            best_params = best_params_cnn_wti
        elif model_type == 'CNN-LSTM':
            best_params = best_params_cnn_lstm_wti
        elif model_type == 'TimesNet':
            best_params = best_params_timesnet_wti
        elif model_type == 'Transformer':
            best_params = best_params_transformer_wti
        else:
            raise ValueError("Invalid model_type. Choose either 'LSTM' or 'CNN'.")
        
    elif data_type == 'gold':
        if model_type == 'LSTM':
            best_params = best_params_lstm_gold
        elif model_type == 'CNN':
            best_params = best_params_cnn_gold
        elif model_type == 'CNN-LSTM':
            best_params = best_params_cnn_lstm_gold
        elif model_type == 'TimesNet':
            best_params = best_params_timesnet_gold
        elif model_type == 'Transformer':
            best_params = best_params_transformer_gold
        else:
            raise ValueError("Invalid model_type. Choose either 'LSTM' or 'CNN'.")
        
    elif data_type == 'us_10yb':
        if model_type == 'LSTM':
            best_params = best_params_lstm_us_10yb
        elif model_type == 'CNN':
            best_params = best_params_cnn_us_10yb
        elif model_type == 'CNN-LSTM':
            best_params = best_params_cnn_lstm_us_10yb
        elif model_type == 'TimesNet':
            best_params = best_params_timesnet_us_10yb
        elif model_type == 'Transformer':
            best_params = best_params_transformer_us_10yb
        else:
            raise ValueError("Invalid model_type. Choose either 'LSTM' or 'CNN'.")
        
    elif data_type == 'nikkei_cme':
        if model_type == 'LSTM':
            best_params = best_params_lstm_nikkei_cme
        elif model_type == 'CNN':
            best_params = best_params_cnn_nikkei_cme
        elif model_type == 'CNN-LSTM':
            best_params = best_params_cnn_lstm_nikkei_cme
        elif model_type == 'TimesNet':
            best_params = best_params_timesnet_nikkei_cme
        elif model_type == 'Transformer':
            best_params = best_params_transformer_nikkei_cme
        else:
            raise ValueError("Invalid model_type. Choose either 'LSTM' or 'CNN'.")
...
def predict(request: PredictionRequest):
    ...
    if data_type == 'nikkei':
        ...
        df_prediction = df_stock_data_usd_jpy[['Adj Close']].tail(1)
        df_prediction.rename(columns={'Adj Close': '終値'}, inplace=True)
        df_prediction = df_prediction.T
        df_prediction.columns = df_prediction.columns.strftime('%Y年%m月%d日')
        df_day_1_5 = pd.DataFrame(predictions_rescaled)
